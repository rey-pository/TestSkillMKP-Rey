...
class CreateMasterDataTokoTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('master_data_toko', function (Blueprint $table) {
            $table->id(); // id (auto increment, unique)
            $table->string('toko_code', 100); // toko_code
            ...
            $table->string('toko_brand', 255); // toko_brand
            $table->string('status', 100); // status
            $table->text('corp_file_siup'); // corp_file_siup
            $table->text('corp_file_npwp'); // corp_file_npwp
            $table->string('pic_phone', 16)->unique(); // pic_phone (unique, mandatory)
            $table->string('pic_email', 100)->unique(); // pic_email (unique, mandatory)
    ...
    {
        Schema::dropIfExists('master_data_toko');
    }
}
